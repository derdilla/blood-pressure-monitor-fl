...
void main() {
  group('ScriptedFormatter', () {
    ...
    test('should create correct strings', () {
      ...
      expect(ScriptedFormatter(r'constant text',).encode(r), 'constant text');
      expect(ScriptedFormatter(r'$SYS',).encode(r), r.sys?.mmHg.toString());
      expect(ScriptedFormatter(r'$DIA',).encode(r), r.dia?.mmHg.toString());
      expect(ScriptedFormatter(r'$PUL',).encode(r), r.pul.toString());
      expect(ScriptedFormatter(r'$COLOR',).encode(r), r.note?.color.toString());
      expect(ScriptedFormatter(r'$NOTE',).encode(r), r.note?.note);
      expect(ScriptedFormatter(r'$TIMESTAMP',).encode(r), r.time.millisecondsSinceEpoch.toString());
      expect(
        ScriptedFormatter(r'$SYS$DIA$PUL',).encode(r),
        (r.sys!.mmHg.toString() + r.dia!.mmHg.toString() + r.pul.toString()),);
      expect(
        ScriptedFormatter(r'$SYS$SYS',).encode(r),
      ...
      expect(
          ScriptedFormatter(r'$SYS-$DIA',).encode(r), ('${r.sys?.mmHg}-${r.dia?.mmHg}'));

      final formatter = DateFormat.yMMMMEEEEd();
      expect(ScriptedFormatter('\$FORMAT{\$TIMESTAMP,${formatter.pattern}}',).encode(r),
    ...
    test('should correctly decode reversible patterns', () {
      ...
      final encodedPurple = ScriptedFormatter(r'$COLOR',).encode(r);
    ...
    test('should when ignore groups in format strings', () {
      ...
      expect(ScriptedFormatter(r'($SYS)',).encode(r1), '(123)');
      expect(ScriptedFormatter(r'($SYS',).encode(r1), '(123');
      final r2 = mockEntry(note: 'test');
      expect(ScriptedFormatter(r'($NOTE',).encode(r2), '(test');
  ...
  group('ScriptedTimeFormatter', () {
    test('should create non-empty string', () {
      final r1 = mockEntry();
      expect(ScriptedTimeFormatter('dd').encode(r1), isNotNull);
      expect(ScriptedTimeFormatter('dd').encode(r1), isNotEmpty);
    });
